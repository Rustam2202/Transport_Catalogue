...
void CalculateBusRequest(string& answer, Node stat_data, TC_Proto::TransportCatalogue& tc);
void CalculateMapRequest(string& answer, MapRenderer& render, Node stat_data, TC_Proto::TransportCatalogue& tc);
void CalculateRouteRequest(string& answer, Node stat_data, TC_Proto::TransportCatalogue& tc);
...
void DeSerialization(std::istream& strm, std::ostream& output) {
	...
	for (const auto& stat_data : base.AsDict().at("stat_requests").AsArray()) {
		if (stat_data.AsDict().at("type").AsString() == "Stop") {
			CalculateStopRequest(answer, stat_data, tc);
		}
		else if (stat_data.AsDict().at("type").AsString() == "Bus") {
			CalculateBusRequest(answer, stat_data, tc);
		}
		else if (stat_data.AsDict().at("type").AsString() == "Map") {
			CalculateMapRequest(answer, render, stat_data, tc);
		}
		else if (stat_data.AsDict().at("type").AsString() == "Route") {
		CalculateRouteRequest(answer, stat_data, tc);
		}
	}
